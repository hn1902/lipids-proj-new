...
with ui.nav_panel("Upload Data"):
    ...
    def validate_inputs():
        ...
        try:
            ...
            for file in input.pos_data():
                chunk = pd.read_csv(file["datapath"], nrows=5)
                df_list.append(chunk)
            head_df = pd.concat(df_list, ignore_index=True)
        except Exception as e:
            return [f"Failed to read data CSVs: {e}"]
        if input.idx_col() is None or input.idx_col() == "":
            errors.append("Index column (lipid species) is not specified.")
        elif input.idx_col() not in head_df.columns:
            errors.append(f"Index column '{input.idx_col()}' not found in data.")
        try:
            nidx = int(input.num_idx()) if input.num_idx() is not None else None
        except Exception:
            nidx = None
        if nidx is None or nidx < 1 or nidx > len(head_df.columns):
            errors.append("Invalid value for 'How many columns contain lipid information?'.")
        if input.header() is not None:
            try:
                for h in input.header():
                    header = pd.read_csv(h['datapath'])
                hdr_index = set(header.T.index)
                data_cols = set(head_df.columns)
                if len(data_cols & hdr_index) == 0:
                    errors.append("Header file does not match any data columns.")
            except Exception as e:
                errors.append(f"Failed to read header CSV: {e}")
        return errors
    ...
    def validation_errors():
        ...
        if errs:
            return "Input issues: " + "; ".join(errs)
        return ""

    with ui.navset_card_underline(title="Dataframes"):   
        @reactive.calc
        def df_func():
            num_col_lvls = input.num_col_lvls()
            if input.pos_data() is None or input.num_idx() is None or num_col_lvls is None or input.idx_col() is None or input.idx_col() == "":
                return

            try:
                nlvls = int(num_col_lvls)
            except Exception:
                nlvls = None
            if nlvls is None:
                return
            if nlvls > 1 and (input.main_col_lvl() is None or input.cohort_lvl() is None or input.drop_col() is None):
                return
            else:
                errs = validate_inputs()
                if errs:
                ...
                def _cohort_from_col(col: str) -> str:
                ...
                if input.header() is None: # use columns in df
                    if nlvls == 1:
                        df_cohort.columns = [_cohort_from_col(c) for c in df_cohort.columns]
                    elif w >= 0: # if w < 0, use original column name
                ...
                try:
                    ...
                    # inputs may not be initialized yet in some reactive contexts; silently skip
                    pass
                ...
                # Ensure abundance columns are numeric for downstream groupby/sum analyses
                for c in df.columns:
                    if c == 'Sample Name':
                        continue
                ...
                if df.empty:
                    return None  # Prevent downstream errors from empty DataFrame
        ...
        @reactive.calc
        def df_exps_func():
            if input.pos_data() is None or input.num_col_lvls() is None or input.num_idx() is None:
                return

            try:
                nlvls = int(input.num_col_lvls())
            except Exception:
                return

            if input.header() is not None:
                for hh in input.header():
                    header = pd.read_csv(hh['datapath'])
            ...
            # No external header: build a synthetic header table from column names.
            first = input.pos_data()[0]
            df0 = pd.read_csv(first["datapath"], nrows=0)
            cols = list(df0.columns)
            cols = cols[int(input.num_idx()):]
            ...
            if nlvls <= 1:
                rows = [cols, [_cohort_from_col(c) for c in cols]]
            else:
                # fall back to the original behavior (expecting header rows embedded in the CSV)
                df_list = []
                for file in input.pos_data():
                    chunk = pd.read_csv(file["datapath"])
                    df_list.append(chunk)
                df = pd.concat(df_list, ignore_index=True)
                df.drop(columns=list(df.columns[:input.num_idx()]), inplace=True)
                header = df[:nlvls-1]
                rows = [list(header.columns)] + [list(r) for _, r in header.iterrows()]

            h = pd.DataFrame(rows)
            h.index = [i + 1 for i in range(len(h.index))]
            h.columns = [str(l) for l in range(0, len(h.columns))]
            h.reset_index(inplace=True)
            return h
...
with ui.nav_panel("Chain Length"):
    ...
    with ui.card(full_screen=True):
        ...
        def cl_fold_change():
            ...
            if df is None or df_meta is None:
                return
            # use existing fold_change utility which will create its own clustermap
            try:
                _df, non_inf_max, non_inf_min = functions.fold_change(df_meta, df, 'Acyl Chain Length', ctrl, row_cluster=True, renamed_var='Acyl Chain Length')
            except Exception as e:
                plt.figure(); plt.text(0.5,0.5,f'Fold change failed: {e}', ha='center'); plt.axis('off'); return plt.gcf()
            return plt.gcf()
...
with ui.nav_panel("Statistics"):
    ...
    with ui.layout_sidebar():
        ...
        with ui.card():
            ui.card_header("Pairwise Comparisons (Holm-Sidak Post-hoc)")
            @render.data_frame
            def posthoc_table():
                res = stats_result()
                if res is None:
