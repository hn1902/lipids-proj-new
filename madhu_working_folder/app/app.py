...
from shiny.download import download_handler
...
def server(input, output, session):
    ...
    with ui.navset_card_underline(title="Dataframes"):   
        @reactive.calc
        def df_func():
            if input.pos_data() is None or input.main_col_lvl() is None or input.cohort_lvl() is None or input.drop_col() is None or input.num_idx() is None or input.num_col_lvls() is None:
                return 
            else:
                errs = validate_inputs()
                if errs:
        ...
        with ui.nav_panel("Filtered Data"):
            with ui.layout_columns(col_widths=(9,3)):
                ...
                def render_filtered_df():
                    ...
                    return df
                with ui.card():
                    ui.download_button("download_filtered_df", "Download filtered dataframe (CSV)")

            @render.download
            def download_filtered_df():
                if df_func() is None:
                    return
                df, _ = df_func()
                def _write(file):
            ...
            with ui.layout_sidebar():
                ...
                with ui.layout_columns(col_widths=(9,3)):
                    ...
                    def render_df_exps():
                        return df_exps_func()
                    with ui.card():
                        ui.download_button("download_header_table", "Download header table (CSV)")

                @render.download
                def download_header_table():
                    h = df_exps_func()
                    if h is None:
                        return
                    def _write(file):
                        h.to_csv(file, index=False)
                    return _write
        ...
        with ui.nav_panel("Row (Lipid) Metadata"):
            with ui.layout_columns(col_widths=(9,3)):
                ...
                def render_df_meta():
                    ...
                    return df_meta_func()
                with ui.card():
                    ui.download_button("download_df_meta", "Download lipid metadata (CSV)")

            @render.download
            def download_df_meta():
                m = df_meta_func()
                if m is None:
                    return
                def _write(file):
        ...
        with ui.nav_panel("Final Dataframe"):
            with ui.layout_columns(col_widths=(9,3)):
                ...
                def render_df():
                    ...
                    return df_display
                with ui.card():
                    ui.download_button("download_final_df", "Download final dataframe (CSV)")

            @render.download
            def download_final_df():
                if df_func() is None:
                    return
                df_display, _ = df_func()
                def _write(file):
                    df_display.to_csv(file, index=False)
                return _write
    ...
    def pca2d():
        ...
        return fig
                    if df_pca is None:
                        return
                    def _write(file):
                        pd.DataFrame({"PC": [1,2,3], "ExplainedVariance": list(ev)}).to_csv(file, index=False)
                    return _write
